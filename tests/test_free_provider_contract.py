...
def test_github_live_provider_self_test_contract(monkeypatch):
    captured = {}

    class FakeResponse:
        headers = {'x-github-request-id': 'live-check-1'}

        def raise_for_status(self):
            return None

        def json(self):
            return {'id': 'completion-1', 'choices': [{'message': {'content': 'READY'}}]}
    ...
    import feedback_intelligence_engine.services.provider_check as module

    monkeypatch.setattr(module.httpx, 'Client', FakeClient)
    result = module.run_provider_self_test(
        Settings(
            github_token='secret-token',
            synthesis_provider='github',
            github_model='openai/gpt-4.1-mini',
    ...
    assert result.llm_operational is True
    assert result.request_id == 'live-check-1'
    assert captured['headers']['Authorization'] == 'Bearer secret-token'
