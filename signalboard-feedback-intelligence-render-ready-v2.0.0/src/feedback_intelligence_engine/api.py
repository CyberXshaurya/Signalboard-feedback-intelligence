...
def list_historical_themes(project_id: str, db: DB, user_id: User) -> list[HistoricalTheme]:
    ...
    return list(
        db.scalars(
            select(HistoricalTheme)
            .where(HistoricalTheme.project_id == project_id)
            .order_by(HistoricalTheme.created_at.desc())
        )
    )


@router.patch(
    "/projects/{project_id}/historical-themes/{historical_theme_id}",
    response_model=HistoricalThemeOut,
)
def update_historical_theme(
    project_id: str,
    historical_theme_id: str,
    payload: HistoricalThemeUpdate,
    db: DB,
    user_id: User,
) -> HistoricalTheme:
    owned_project(db, project_id, user_id)
    item = db.scalar(
        select(HistoricalTheme).where(
            HistoricalTheme.id == historical_theme_id,
            HistoricalTheme.project_id == project_id,
        )
    )
    if item is None:
        raise HTTPException(status_code=404, detail="Historical theme not found")
    updates = payload.model_dump(exclude_unset=True)
    if not updates:
        raise HTTPException(status_code=422, detail="No historical theme changes were supplied")
...
def delete_historical_theme(
    project_id: str, historical_theme_id: str, db: DB, user_id: User
) -> None:
    owned_project(db, project_id, user_id)
    item = db.scalar(
        select(HistoricalTheme).where(
...
@router.post("/providers/self-test", response_model=ProviderSelfTestOut)
def provider_self_test(settings: SettingsDep) -> ProviderSelfTestOut:
    from .services.provider_check import run_provider_self_test

    try:
        return run_provider_self_test(settings)
    except Exception as exc:
        raise HTTPException(
            status_code=503,
            detail=f"Live provider check failed: {type(exc).__name__}: {exc}",
        ) from exc
